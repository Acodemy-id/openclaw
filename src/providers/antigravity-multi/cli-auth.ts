...
<html lang="en">
  ...
  <body>
    <main>
      ...
      <p>You can return to the terminal and copy the authorization code if needed.</p>
    </main>
  </body>
</html>`;

export interface AuthParams {
    openUrl?: (url: string) => Promise<void>; // Add mechanism to open browser
    log?: (msg: string) => void;
}
...
async function startCallbackServer(params: { timeoutMs: number }) {
    ...
    const server = createServer((request, response) => {
        ...
        // Loose check for path to handle potential differences
        if (!url.pathname.includes(redirect.pathname)) {
            // Wait, standard match
        }
...
export async function exchangeCode(params: {
    ...
    // Manually handle different code formats if pasted (e.g. sometimes they have "4/" prefix, sometimes full URL)
    // For now assuming clean code
...
export async function refreshAccessToken(refreshToken: string): Promise<{ access: string; expires: number }> {
    const response = await fetch(TOKEN_URL, {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: new URLSearchParams({
...
export class CliAuthenticator {
    async performLogin(): Promise<{
        ...
        verifier: string;
        state: string;
        callbackPromise?: Promise<URL>; // If local server is started
        ...
        try {
            ...
            // Fallback to manual if port blocked or whatever
            callbackServer = null;
        }
        ...
        return {
            ...
            verifier,
            state,
            callbackPromise: callbackServer?.waitForCallback(),
